...
% ADDITIONS TO spm_searchlight
%    .model - model name, defaulting to 'searchlight'
%    .maps  - a cell array of names for the individual maps  //ADDITIONS
...
% additions
if ~isfield(searchopt, 'outputdir') || isempty(searchopt.outputdir)
    utils = rsa_utils();
    searchopt.outputdir = utils.get_commonpath(char(VY(:).fname));
end
if ~isfield(searchopt, 'model') || isempty(searchopt.model)
    searchopt.model = 'searchlight';
end
if ~isfield(searchopt, 'maps')
    searchopt.maps = [];
end
...
for i=1:size(SLR,2)
    if isempty(searchopt.maps)
        VO(i).fname = fullfile(searchopt.outputdir,sprintf('%s_%04d%s', ...
            searchopt.model,i,spm_file_ext));
    else
        VO(i).fname = fullfile(searchopt.outputdir, sprintf('%s_%s%s', ...
            searchopt.model,searchopt.maps{i},spm_file_ext));
    end        
    VO(i).descrip = sprintf('%s (%04d)',VO(i).descrip,i);
end
if ~exist(searchopt.outputdir, 'dir'), mkdir(searchopt.outputdir); end
for i=1:size(SLR,2)
